...
10. 主送机关属于表单维护的公文元数据，正文任何位置都不得出现主送机关称谓或其后的冒号。不得生成或保留类似“各县（市、区）农业农村局，泉州开发区社会事业局，泉州台商投资区民生保障局：”这样的收文单位行；即使 source_document 或 writing_instruction 中包含主送机关，也必须删除，不得照抄。通知正文应直接从导语段或章节标题开始。
11. 改写完整文档时，除非 writing_instruction 明确要求，否则保留原文事实、数据、段落顺序和核心观点；无论 source_document 原先使用什么格式，都必须将最终结果规范为一个 h1、若干 h2 和普通段落。
12. 创作新文档时，根据 writing_instruction 生成结构完整的全文，不要输出大纲、占位符或未完成片段。
13. 如果提供了 style_profile，必须将其稳定地体现在结构、句式、节奏和表达习惯中，但不得凭空改变原文事实。
